...
import { clamp, floorMod } from "./math";
import µ from './micro';
...
function atlantis() {
    return newGlobe({
        newProjection: function() {
            return d3Geo.geoMollweide().rotate([30, -45, 90]).precision(0.1);
        }
    });
}
...
function orthographic() {
    return newGlobe({
        newProjection: function() {
            return d3.geoOrthographic().rotate(currentPosition()).precision(0.1);
        },
        defineMap: function(mapSvg, foregroundSvg) {
            var path = d3.geoPath(this.projection);
            var defs = mapSvg.append("defs");
...
function waterman() {
    return newGlobe({
        newProjection: function() {
            return d3Geo.geoPolyhedralWaterman().rotate([20, 0]).precision(0.1);
        },
        defineMap: function(mapSvg, foregroundSvg) {
            var path = d3.geoPath(this.projection);
            var defs = mapSvg.append("defs");
...
function winkel3() {
    return newGlobe({
        newProjection: function() {
            return d3Geo.geoWinkel3().precision(0.1);
        }
    });
}
